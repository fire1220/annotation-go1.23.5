...
type mspan struct {
	...
	// 有效空闲下标范围是 0 <= freeindex < melems
	// 当freeindex = melems时表span没有可用块（无空用内存）
	freeindex uint16 // 空闲对象块的下标位置（下标范围是[0 - nelems]之间(包含nelems),最大有效数是nelems-1,）空闲内存地址是 s.freeindex * s.elemsize + s.base()
	...
	// 译：
	// 清理阶段
	// 如果sweepgen == h->sweepgen - 2，【需要清理、未缓存】span(跨度)需要清扫标记。(理解为重新清扫一下，因为每次清扫完成后会自动加2)
	// 如果sweepgen == h->sweepgen - 1，【正在清理、未缓存】span(跨度)正在扫描标记。(理解为重新清理过程中的状态，从新清理是-2,过程中会+1，所以状态为-1)
...
func (h *mheap) initSpan(s *mspan, typ spanAllocType, spanclass spanClass, base, npages uintptr) {
	...
	if typ.manual() {
		...
		s.allocCache = ^uint64(0) // all 1s indicating all free. // “all 1s” 指的是“所有对象初始时都已分配”
